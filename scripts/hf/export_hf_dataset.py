...
def build_record(prepared_dir: Path, diff_cap: int = 256_000, notes_cap: int = 256_000) -> Dict[str, Any]:
    ...
    record: Dict[str, Any] = {
        ...
        "artifacts": {
            ...
            "notes": notes,
            "repo_info": repo_info_text,
            "paths": paths,
        },
        "meta": {
            "prepared_version": "one_shot_bench",
            "created_from": str(prepared_dir),
        },
    }
    return record
...
def export_dataset(out_path: Path, split: Optional[str] = None, validate: bool = False) -> int:
    ...
    with out_path.open("w", encoding="utf-8") as f:
        for p in prepared_dirs:
            rec = build_record(p)
            # Optional split support via hash
            if split:
...
#!/usr/bin/env python3
"""
Export prepared tasks to HuggingFace dataset format.
Converts data/tasks/prepared/* to JSONL.
"""
