...
Examples:
  # Run evaluation with overrides
  uv run python -m spec_bench.cli run-with-overrides ./task ./overrides.json

  # Quick evaluation with custom OpenAI endpoint
  uv run python -m spec_bench.cli quick-eval ./task --model gpt-4 --openai-base-url https://eu.api.openai.com/v1

  # Batch evaluation from config
  uv run python -m spec_bench.cli batch-eval ./config.toml

  # Set up custom provider
  uv run python -m spec_bench.cli setup-provider myproxy https://my-proxy.example.com/v1

  # Validate setup
  uv run python -m spec_bench.cli validate ./task ./overrides.json
